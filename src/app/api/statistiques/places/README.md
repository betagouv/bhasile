...
- Un objet par département présent dans le périmètre filtré.
- `places` : somme des `placesAutorisees` résolues (global) des structures du département ayant une typologie.
- `population` : population du département (`Departement.population`).
- `tauxPour1000` : `(places / population) × 1000`, ou `null` si population absente.
...
- Même formule que le global, avec les places du millésime exact.
