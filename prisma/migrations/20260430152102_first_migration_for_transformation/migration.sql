/*
  ...
  - A unique constraint covering the columns `[structureId,structureTransformationId,year]` on the table `StructureMillesime` will be added. If there are existing duplicate values, this will fail.
  - A unique constraint covering the columns `[structureId,structureTransformationId,year]` on the table `StructureTypologie` will be added. If there are existing duplicate values, this will fail.

*/
-- CreateEnum
CREATE TYPE "TransformationType" AS ENUM ('CREATION', 'EXTENSION_ET_CREATION', 'FERMETURE');

-- CreateEnum
CREATE TYPE "TransformationSubtype" AS ENUM ('OUVERTURE_EX_NIHILO', 'OUVERTURE_DEPUIS_UNE_OU_PLUSIEURS_STRUCTURES', 'EXTENSION_FROM_SCRATCH', 'EXTENSION_DEPUIS_STRUCTURES_QUI_CONTRACTENT', 'EXTENSION_DEPUIS_STRUCTURES_QUI_FERMENT', 'CONTRACTION_AVEC_TRANSFERT_VERS_AUTRE_STRUCTURE', 'CONTRACTION_SANS_TRANSFERT_DE_PLACES', 'FERMETURE_AVEC_TRANSFERT_VERS_UNE_OU_PLUSIEURS_STRUCTURES', 'FERMETURE_SANS_TRANSFERT', 'TRANSFO_HUDA_VERS_CADA_EXISTANT_MEME_OPERATEUR', 'TRANSFO_HUDA_VERS_CADA_NOUVEAU_MEME_OPERATEUR', 'TRANSFO_HUDA_REMISE_EN_CONCURRENCE_DES_PLACES');

-- CreateEnum
CREATE TYPE "StructureTransformationType" AS ENUM ('CREATION', 'FERMETURE', 'CONTRACTION', 'EXTENSION');

-- DropIndex
DROP INDEX "StructureMillesime_structureId_year_key";

-- DropIndex
DROP INDEX "StructureTypologie_structureId_year_key";
...
CREATE TABLE "Transformation" (
    ...
    "subtype" "TransformationSubtype" NOT NULL,
...
-- CreateIndex
CREATE UNIQUE INDEX "StructureMillesime_structureId_structureTransformationId_ye_key" ON "StructureMillesime"("structureId", "structureTransformationId", "year");

-- CreateIndex
CREATE UNIQUE INDEX "StructureTypologie_structureId_structureTransformationId_ye_key" ON "StructureTypologie"("structureId", "structureTransformationId", "year");
...
-- AddForeignKey
ALTER TABLE "Transformation" ADD CONSTRAINT "Transformation_formId_fkey" FOREIGN KEY ("formId") REFERENCES "Form"("id") ON DELETE RESTRICT ON UPDATE CASCADE;
