...
import { getFinanceTransformationMarkers } from "@/app/(authenticated)/(with-menu)/structures/[id]/_components/_finances/getFinanceTransformationMarkers";
...
describe("getFinanceTransformationMarkers", () => {
  it("returns [] for missing or empty input", () => {
    expect(getFinanceTransformationMarkers(undefined, years)).toEqual([]);
    expect(getFinanceTransformationMarkers([], years)).toEqual([]);
    expect(getFinanceTransformationMarkers([extension("2023-05-01")], [])).toEqual(
      []
    );
  });
  ...
  it("keeps only extension/contraction events", () => {
    ...
    const markers = getFinanceTransformationMarkers(history, years);
  ...
  it("groups same-year events, sorts them ascending, and flags MIXED", () => {
    const history = [
      contraction("2023-09-15"),
      extension("2023-02-10"),
    ];
  ...
  it("returns markers ordered by year", () => {
    const markers = getFinanceTransformationMarkers(
