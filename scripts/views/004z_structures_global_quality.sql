...
SELECT sc."id" AS "id",
  ...
  COALESCE(
    cal."has_issue_authorized_convention_missing_or_expired",
    FALSE
  ) AS "has_issue_authorized_convention_missing_or_expired",
  -- Calendar: convention dates differ from Actes Administratifs (when present)
  COALESCE(
    cal."has_issue_convention_dates_differ_from_actes_administratifs",
    FALSE
  ...
  COALESCE(
    cal."has_issue_authorisation_dates_differ_from_actes_administratifs",
    FALSE
  ) AS "has_issue_authorisation_dates_differ_from_actes_administratifs",
  -- Calendar: evaluation not done in time
  COALESCE(
    cal."has_issue_evaluation_not_done_in_time",
    FALSE
  ) AS "has_issue_evaluation_not_done_in_time",
  ...
  COALESCE(
    fin."has_issue_taux_encadrement_max_gt_threshold",
    FALSE
  ) AS "has_issue_taux_encadrement_max_gt_threshold",
  ...
  COALESCE(
    ...
    FALSE
  ) AS "has_issue_cout_journalier_max_gt_tarif_cible",
  ...
  COALESCE(
    fin."has_issue_authorized_reprise_wrong_sign",
    FALSE
  ) AS "has_issue_authorized_reprise_wrong_sign",
  -- Finance (subsidized): RN < 0 but excedentRecupere/excedentDeduit/fondsDedies are non-zero
  COALESCE(
    fin."has_issue_subsidized_deficit_nonzero_boxes",
    FALSE
  ) AS "has_issue_subsidized_deficit_nonzero_boxes",
  -- Finance (subsidized): RN > 0 but repriseEtat is non-zero
  COALESCE(
    fin."has_issue_subsidized_reprise_etat_nonzero",
    FALSE
  ...
  (
    (
    ...
    ) + (COALESCE(ch."has_issue_dept_code", FALSE)::int) + (COALESCE(ch."has_issue_multi_dna", FALSE)::int) + (
      COALESCE(ch."has_issue_cpom_mono_structure", FALSE)::int
    ) + (
      COALESCE(
        fin."has_issue_taux_encadrement_max_gt_threshold",
        FALSE
      ...
      COALESCE(fin."has_issue_taux_encadrement_min_lt_2", FALSE)::int
    ) + (
      COALESCE(
        fin."has_issue_cout_journalier_max_gt_tarif_cible",
        FALSE
      ...
      COALESCE(doc."has_issue_missing_cpom_document", FALSE)::int
    ) + (
      COALESCE(
        act."has_issue_places_indisponibles_gt_3pct",
        FALSE
      ...
      COALESCE(act."has_issue_presences_indues_gt_7pct", FALSE)::int
    )
  ) AS "issues_count"
