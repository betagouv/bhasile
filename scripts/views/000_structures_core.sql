...
CREATE OR REPLACE VIEW :"SCHEMA"."structures_core" AS WITH structure_version_latest AS (
...
SELECT s."id" AS "id",
  ...
  COALESCE(
    svl."departementAdministratif",
    s."departementAdministratif"
  ) AS "departement_administratif",
...
FROM public."Structure" s
  ...
  LEFT JOIN public."Departement" dep ON dep."numero" = COALESCE(
    svl."departementAdministratif",
    s."departementAdministratif"
  )
