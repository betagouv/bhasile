...
CREATE OR REPLACE VIEW :"SCHEMA"."structures_finance_quality" AS WITH structures AS (
  ...
  tarifs AS (
    SELECT si."id" AS "structureId",
      t."tarifCible"
    FROM structures_with_idf_boolean si
  ...
  budgets_rates AS (
    SELECT i."structureId" AS "structureId",
    ...
    FROM indicateurs_financiers_filtered i
    GROUP BY i."structureId"
  ),
...
SELECT s."id" AS "id",
  ...
  COALESCE(
    br."cout_journalier_max" > tc."tarifCible",
    FALSE
  ...
  COALESCE(
    bi."has_issue_authorized_reprise_wrong_sign",
    FALSE
  ) AS "has_issue_authorized_reprise_wrong_sign",
  COALESCE(
  ...
  COALESCE(
    bi."has_issue_subsidized_reprise_etat_nonzero",
    FALSE
  ) AS "has_issue_subsidized_reprise_etat_nonzero",
